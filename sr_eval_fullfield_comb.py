...
def plot_full_maps(out_dir: str, day_str: str, var_name: str, hour_idx: int,
    ...
    Inputs (already in chosen scale): [B=1, C, T, H, W].
    """
    ...
    hr_img = hr_c[0] if getattr(hr_c, "ndim", 0) == 3 else hr_c
    ...
    for ax, (title, im) in zip(axs, panels):
        ax.imshow(im, vmin=vmin, vmax=vmax, cmap = 'jet')
        ax.colorbar()
        ax.set_title(title)
        ax.axis("off")
...
def main():
    ...
    for di, p in enumerate(test_files):
        ...
        temp = temporal_interp(x, factor=args.temporal_factor)
        ...
        # Ensure all tensors match y’s spatial size (H, W)
        ...
        if metrics_sum_std is not None:
            ...
            # B, C, Tt, H, W = y_std.shape
            # def bt(t): return t.reshape(B * Tt, C, H, W)
